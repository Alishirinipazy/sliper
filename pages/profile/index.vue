      ...
      <UForm :schema="schema" :state="formState" @submit="onSubmit" class="profile-form space-y-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <UFormGroup label="ایمیل" name="email" required>
            <UInput v-model="formState.email" type="email" placeholder="example@email.com" icon="i-heroicons-envelope" dir="ltr" />
          </UFormGroup>
        ...
        <div class="flex justify-end sm:justify-start">
          <UButton type="submit" :loading="loading" icon="i-heroicons-check" label="ذخیره تغییرات" class="bg-mainColor text-secColor font-bold" />
        </div>
      </UForm>
    </UCard>
  </div>
...
<style scoped>
.profile-form-page { min-height: calc(100vh - 2rem); }
.profile-form-card { background: linear-gradient(150deg, rgba(7,27,36,.94), rgba(28,27,34,.94)); border: 1px solid rgba(255,190,51,.14); color: white; }
.profile-form :deep(label) { color: rgba(255,255,255,.78); }
...
.profile-form :deep(input::placeholder), .profile-form :deep(textarea::placeholder) { color: rgba(255,255,255,.35) !important; }
.profile-form :deep(input:focus), .profile-form :deep(textarea:focus), .profile-form :deep(select:focus) { border-color: #ffbe33 !important; box-shadow: 0 0 0 2px rgba(255,190,51,.10) !important; }
.profile-form :deep(input:disabled) { background: rgba(255,255,255,.055) !important; color: rgba(255,255,255,.48) !important; }
@media (max-width: 639px) {
  .profile-form-page { padding: .75rem !important; }
  .profile-form-card :deep(.p-6), .profile-form-card :deep(.sm\:p-8) { padding: 1rem !important; }
  .profile-form :deep(input), .profile-form :deep(textarea), .profile-form :deep(select) { min-height: 46px; border-radius: 14px; }
  .profile-form > div:last-child, .profile-form .flex.justify-end { width: 100%; }
  .profile-form .flex.justify-end :deep(button) { width: 100%; min-height: 46px; }
}
</style>