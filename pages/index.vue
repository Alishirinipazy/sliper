...
<style>
.aquarium-home { @apply relative min-h-screen overflow-hidden; }
.ocean-section { @apply relative my-5 rounded-[30px] p-1 sm:p-2; }
.section-glow { @apply absolute inset-x-10 -top-10 h-24 rounded-full bg-cyan-400/10 blur-3xl pointer-events-none; }
.aquarium-orb { position: absolute; border-radius: 9999px; filter: blur(55px); pointer-events: none; opacity: .35; }
.orb-a { width: 24rem; height: 24rem; top: 18rem; right: -12rem; background: rgba(49,216,255,.20); }
.orb-b { width: 20rem; height: 20rem; top: 60rem; left: -10rem; background: rgba(15,111,146,.24); }
.aquarium-bubbles { position: absolute; inset: 0; pointer-events: none; overflow: hidden; opacity: .55; }
.aquarium-bubbles span { position: absolute; bottom: -30px; right: calc((var(--i) * 7%) - 3%); width: calc(5px + (var(--i) % 4) * 2px); height: calc(5px + (var(--i) % 4) * 2px); border: 1px solid rgba(178,244,255,.45); border-radius: 50%; box-shadow: 0 0 10px rgba(108,231,255,.25); animation: bubbleRise calc(11s + var(--i) * .55s) linear infinite; animation-delay: calc(var(--i) * -1.1s); }
@keyframes bubbleRise { 0% { transform: translate3d(0, 0, 0) scale(.75); opacity: 0; } 12% { opacity: .7; } 55% { transform: translate3d(-18px, -45vh, 0) scale(1); } 100% { transform: translate3d(22px, -110vh, 0) scale(1.15); opacity: 0; } }
@media (prefers-reduced-motion: reduce) { .aquarium-bubbles span { animation: none; } }
</style>