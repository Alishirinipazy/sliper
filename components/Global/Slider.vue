...
function openStory(i: number) { viewerIndex.value = i; viewerOpen.value = true }
...
const slideStyle = (item: any) => ({
  backgroundImage: `linear-gradient(90deg, rgba(15,16,21,.86), rgba(28,27,34,.28) 58%, rgba(17,18,22,.68)), url(${item?.file || item?.image || ''})`
})
</script>

<template>
  <div v-if="loading"><USkeleton class="h-[70vh]" :ui="{ background: 'bg-[#171820]' }" /></div>
  ...
  <div v-else class="relative z-0 isolate px-2 pt-2 lg:px-3">
            ...
            <div class="slider-body aqua-surface" :class="h" :style="slideStyle(item)">
              ...
              <div class="slider-copy">
                ...
                <div v-if="item?.link && sliderText(item).cta" class="flex flex-wrap justify-center gap-3 sm:justify-start">
                  <NuxtLink :to="item.link" class="slider-cta">{{ sliderText(item).cta }} <UIcon name="material-symbols:arrow-back-rounded" /></NuxtLink>
                  <NuxtLink v-if="sliderText(item).secondaryCta" to="/products" class="slider-ghost">{{ sliderText(item).secondaryCta }}</NuxtLink>
                </div>
              </div>
            </div>
          </swiper-slide>
    ...
    <div v-else class="slider-body aqua-surface" :class="h" :style="slideStyle(items)">
      <div class="underwater-grid" aria-hidden="true"></div>
      <div class="water-light light-one" aria-hidden="true"></div>
      <div class="water-light light-two" aria-hidden="true"></div>
      <div class="surface-shimmer" aria-hidden="true"></div>
      <div class="slider-copy">
        ...
        <NuxtLink v-if="items?.link && sliderText(items).cta" :to="items.link" class="slider-cta">{{ sliderText(items).cta }} <UIcon name="material-symbols:arrow-back-rounded" /></NuxtLink>
      </div>
    </div>
    ...
    <div v-if="stories?.data?.length" class="story-dock aqua-glass">
      <div class="overflow-x-auto" style="scrollbar-width: none">
        <div class="flex min-w-max gap-4 px-3 py-2">
          <div v-for="(story, i) in stories.data" :key="story.id || i" class="group flex w-16 cursor-pointer flex-col items-center gap-1" @click="openStory(i)">
            <div class="story-ring"><div class="h-12 w-12 overflow-hidden rounded-full border border-white/20 bg-[#111216]">
...
<style>
.slider-body { @apply relative z-0 flex w-full items-center overflow-hidden rounded-[28px] bg-cover bg-center px-7 py-16 text-white sm:px-12 lg:rounded-[36px] lg:px-20; min-height:min(72vh,760px); box-shadow:0 30px 90px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.14); isolation:isolate; }
.slider-copy { @apply relative z-30 max-w-2xl text-center sm:text-right; }
.eyebrow { @apply mb-4 inline-flex items-center rounded-full border border-mainColor/20 bg-mainColor/10 px-4 py-2 text-xs backdrop-blur-md sm:text-sm; color:color-mix(in srgb,var(--main-color) 85%,white); }
.slider-title { @apply py-2 text-3xl font-black leading-tight tracking-tight sm:text-4xl lg:text-6xl; text-shadow:0 8px 30px rgba(0,0,0,.35); }
.slider-subtitle { @apply mb-6 max-w-xl text-sm leading-7 text-white/75 sm:text-base; }
.slider-cta { @apply relative z-40 inline-flex items-center gap-2 rounded-full bg-mainColor px-6 py-3 font-bold text-secColor transition hover:-translate-y-1; box-shadow:0 0 30px rgba(255,190,51,.28); }
.slider-ghost { @apply relative z-40 inline-flex rounded-full border border-mainColor/20 bg-white/10 px-6 py-3 font-semibold text-white backdrop-blur-md transition hover:border-mainColor/40 hover:bg-mainColor/10; }
.story-dock { @apply absolute bottom-3 left-1/2 w-[calc(100%-1.5rem)] max-w-4xl -translate-x-1/2 overflow-hidden rounded-[22px]; z-index:50; }
.story-ring { @apply rounded-full p-[2px]; background:linear-gradient(135deg,var(--main-color),color-mix(in srgb,var(--main-color) 45%,transparent),rgba(255,255,255,.3)); }
.water-light { position:absolute; z-index:2; width:24rem; height:24rem; border-radius:50%; filter:blur(70px); background:rgba(255,190,51,.12); pointer-events:none; animation:waterDrift 8s ease-in-out infinite alternate; }
.light-one { top:-8rem; left:-6rem; }.light-two { right:10%; bottom:-12rem; animation-delay:-3s; }
.underwater-grid { position:absolute; z-index:3; inset:0; pointer-events:none; opacity:.12; background:repeating-linear-gradient(105deg,transparent 0 46px,rgba(255,190,51,.18) 48px 49px,transparent 51px 92px); mask-image:linear-gradient(to bottom,rgba(0,0,0,.7),transparent 75%); animation:caustics 11s ease-in-out infinite alternate; }
