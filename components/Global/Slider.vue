          ...
          <swiper-slide v-for="(item, idx) in items.data" :key="item?.id || idx" class="relative z-0">
            <div class="slider-body aqua-surface" :class="h" :style="slideStyle(item)">
              ...
              <div class="slider-copy">
                ...
                <div v-if="item?.link && sliderText(item).cta"
                     class="flex flex-wrap justify-center gap-3 sm:justify-start">
                  <NuxtLink :to="item.link" class="slider-cta">{{ sliderText(item).cta }}
    ...
    <div v-if="stories?.data?.length" class="story-dock aqua-glass">
      <div class="overflow-x-auto" style="scrollbar-width: none">
        <div class="flex min-w-max gap-4 px-3 py-2">
          <div v-for="(story, i) in stories.data" :key="story.id || i"
               class="group flex w-16 cursor-pointer flex-col items-center gap-1" @click="openStory(i)">
            <div class="story-ring">
              <div class="h-12 w-12 overflow-hidden rounded-full border border-white/20 bg-[#111216]">
                ...
                <img v-else-if="story.thumbnail" :src="story.thumbnail" :alt="story.title || 'استوری'"
                     class="h-full w-full object-cover transition duration-500 group-hover:scale-110"/>
                <div v-else class="flex h-full w-full items-center justify-center text-mainColor">
                  <UIcon name="material-symbols:play-arrow" class="h-6 w-6"/>
                </div>
              </div>
            </div>
      ...
      <ClientOnly>
        <StoryViewer v-if="viewerOpen" :stories="stories?.data ?? []" :start-index="viewerIndex"
...
<style>
.slider-body {
  @apply relative z-0 flex w-full items-end justify-center md:justify-start overflow-hidden rounded-[28px] bg-cover bg-center px-7 py-5 text-white sm:px-12 lg:rounded-[36px] lg:px-20;
  min-height: min(10vh, 650px);
...
.eyebrow {
  @apply mb-4 inline-flex items-center rounded-full border border-mainColor/20 bg-mainColor/10 px-4 py-2 text-xs backdrop-blur-md sm:text-sm;
...
.slider-subtitle {
  @apply mb-6 max-w-xl text-sm leading-7 text-white/75 sm:text-base;
}

.slider-cta {
  @apply relative z-40 inline-flex items-center gap-2 rounded-full bg-mainColor px-6 py-3 font-bold text-secColor transition hover:-translate-y-1;
  box-shadow: 0 0 30px rgba(255, 190, 51, .28);
}
...
.story-dock {
  @apply absolute bottom-3 left-1/2 w-auto max-w-4xl -translate-x-1/2 overflow-hidden rounded-[22px];
  z-index: 50;
}
...
.underwater-grid {
  ...
  pointer-events: none;
  opacity: .12;
  background: repeating-linear-gradient(105deg, transparent 0 46px, rgba(255, 190, 51, .18) 48px 49px, transparent 51px 92px);
...
@keyframes waterDrift {
  from {
    transform: translate3d(-3%, 0, 0) scale(.9);
    opacity: .45
  }
  to {
...
@media (prefers-reduced-motion: reduce) {
  .underwater-grid, .surface-shimmer, .water-light {
    animation: none
  }
}
</style>